...
Vieles mag im ersten Schritt kryptisch aussehen, aber am Ende des Tages sind es recht einfach Übungen:
...
Der `mutate()` ist schon komplizierter, aber eigentlich bauen wir uns nur eine Variable die sagt in welcher Zeile im Buch der Text steht (`linenumber = row_number()`).
...
Das alles speichern wir in `original_books()`.
